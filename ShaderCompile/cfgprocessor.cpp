...
public:
	...
	ComboHandleImpl( const ComboHandleImpl& ) noexcept = default;
...
ComboHandle Combo_GetCombo( uint64_t iCommandNumber )
{
	...
	CPCHI_t* pImpl = new( std::nothrow ) CPCHI_t( chiFound );
...
ComboHandle Combo_GetNext( uint64_t& riCommandNumber, ComboHandle& rhCombo, uint64_t iCommandEnd )
{
	...
	if ( !rhCombo )
	{
		...
		// Advance the handle as needed
		pImpl   = new( std::nothrow ) CPCHI_t( chiFound );
	...
	for ( ;; )
	{
		...
		pImpl   = new( std::nothrow ) CPCHI_t( chiNext );
		rhCombo = AsHandle( pImpl );
...
{
	if ( hComboCopyFrom )
		return AsHandle( new( std::nothrow ) CPCHI_t( *FromHandle( hComboCopyFrom ) ) );
