...
/* ################################################################### */
static void ToU1(std::string ** strings, unsigned char * out, size_t n) {
  for(size_t i=0; i < n; ++i) {
    out[i] = (unsigned char)atoi(strings[i]->c_str());
  }
};
/* ################################################################### */
static void ToS1(std::string ** strings, char * out, size_t n) {
  for(size_t i=0; i < n; ++i) {
    out[i] = (char)atoi(strings[i]->c_str());
  }
};
/* ################################################################### */
static void ToU2(std::string ** strings, unsigned short * out, size_t n) {
  for(size_t i=0; i < n; ++i) {
    out[i] = (unsigned short)atoi(strings[i]->c_str());
  }
};
/* ################################################################### */
static void ToS2(std::string ** strings, short * out, size_t n) {
  for(size_t i=0; i < n; ++i) {
    out[i] = (short)atoi(strings[i]->c_str());
  }
};
/* ################################################################### */
static void ToS4(std::string ** strings, int * out, size_t n) {
  for(size_t i=0; i < n; ++i) {
    out[i] = atoi(strings[i]->c_str());
  }
};
/* ################################################################### */
static void ToU4(std::string ** strings, unsigned int * out, size_t n) {
  for(size_t i=0; i < n; ++i) {
    out[i] = (unsigned int)strtoul(strings[i]->c_str(), NULL, 0);
  }
};
/* ################################################################### */
static void ToS8(std::string ** strings, long long * out, size_t n) {
  for(size_t i=0; i < n; ++i) {
...
/* ################################################################### */
static void ToU8(std::string ** strings, unsigned long long * out, size_t n) {
  for(size_t i=0; i < n; ++i) {
    std::stringstream ss(strings[i]->c_str());
    ss >> out[i];
  }
...
/* ################################################################### */
static void ToF(std::string ** strings, float * out, size_t n) {
  for(size_t i=0; i < n; ++i) {
    out[i] = (float)atof(strings[i]->c_str());
  }
};
/* ################################################################### */
static void ToD(std::string ** strings, double * out, size_t n) {
  for(size_t i=0; i < n; ++i) {
...
public:
  ...
  std::map< std::string, size_t > optionGroupIds;
  ...
  std::map< size_t, int > groupValidators;
};
/* ################################################################### */
...
void ezOptionParser::getUsageDescriptions(std::string & usage, int width, Layout layout) {
  ...
  std::map<std::string*, size_t> stringPtrToIndexMap;
  std::vector<std::string* > stringPtrs(groups.size());
