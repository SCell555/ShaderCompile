...
#define IN_CHARACTERSET( SetBuffer, character ) ( ( SetBuffer ).set[static_cast<unsigned char>( character )] )
...
public:
	...
	[[nodiscard]] int GetDelimiterLength() const;
...
public:
	...
	using UtlBufferOverflowFunc_t = bool ( CUtlBuffer::* )( int );
	...
	CUtlBuffer( void* pBuffer, int size, int nFlags = 0 );
	...
	[[nodiscard]] const void* PeekGet( int offset = 0 ) const;
	[[nodiscard]] const void* PeekGet( int nMaxSize, int nOffset );

	// Where am I writing (put)/reading (get)?
	[[nodiscard]] int TellPut() const;
	[[nodiscard]] int TellGet() const;

	// What's the most I've ever written?
	[[nodiscard]] int TellMaxPut() const;

	// How many bytes remain to be read?
	// NOTE: This is not accurate for streaming text files; it overshoots
	[[nodiscard]] int GetBytesRemaining() const;
...
public:
	...
	//
	// If end of file is reached or upon error returns nullptr.
	//
	// Note:	the pointer returned points into the local memory of this buffer, in
	//			case the buffer gets relocated or destroyed the pointer becomes invalid.
	//
	...
	//			nullptr							when EOF is reached or error occurs
	//
...
inline bool CUtlBuffer::WasLastCharacterCR()
{
	if ( !IsText() || ( TellPut() == 0 ) )
		return false;
	return *static_cast<const char*>( PeekPut( -1 ) ) == '\n';
}
